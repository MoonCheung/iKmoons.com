<template>
  <div :class="[isMobile ? 'mobileSearch-wrap' : 'search-wrap']">
    <div class="search-main">
      ...
      <article class="search-list" v-if="!keyWordList.length">
        <div class="container">
      ...
      <article v-else class="search-list">
        <div class="container">
          <div class="media" v-for="(item, index) in keyWordList" :key="index">
          ...
          <button v-if="!noMore" class="btn" :class="searchLoadMore" @click="fetchMoreKeyWord">
            下一页
          </button>
...
export default {
  ...
  scrollToTop: true,
  fetch({ store, params }) {
    return Promise.all([store.dispatch('search/fetchAtrKeyWord', params)]);
  },
  head() {
    const { $route } = this;
    return {
