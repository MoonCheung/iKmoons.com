    ...
    <div class="tags-main">
      <header class="tags-head">
        ...
        <div class="head-count">
          共搜索到&nbsp;{{ apptTags.tagNum === undefined ? 0 : apptTags.tagNum.count }}&nbsp;篇文章
        </div>
      </header>
      <article class="tags-list" v-if="apptTags.tagNum === undefined">
        <div class="container">
      ...
      <article v-else class="tags-list">
        <div class="container">
          <div class="media" v-for="(item, index) in apptTagList" :key="index">
          ...
          <button v-if="!noMore" class="btn" :class="tagsLoadMore" @click="fetchMoreTags">
            下一页
          </button>
...
export default {
  ...
  scrollToTop: true,
  fetch({ store, params }) {
    return Promise.all([store.dispatch('tags/fetchApptTags', params)]);
  },
  head() {
    const { $route } = this;
    return {
...
.tags {
  ...
  &-main {
    display: flex;
    padding: 0.428rem 0.428rem 0 0.428rem;
